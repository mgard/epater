...
\title{Manuel d'utilisation du simulateur ARM\\Révision 1}


\author{Jessica Déziel\\Marc-André Gardner\\Yannick Hold-Geoffroy\\Jean-François Lalonde}
...
Le simulateur est disponible en ligne, à l'adresse \url{http://gif1001-sim.gel.ulaval.ca/}. Aucune installation n'est nécessaire et l'accès ne requiert pas d'authentification. Le simulateur est disponible 24 heures sur 24, sans interruption. Le simulateur a été testé avec les navigateurs suivants :
\begin{itemize}
	...
	\item Microsoft Edge et Microsoft Internet Explorer 11, sur Windows 10
...
Cette interface diffère selon le mode actuel. Dans le mode \emph{d'édition} (figure\ref{f:controles1}), seul le bouton \textit{Démarrer} est actif. Dans le mode \emph{d'exécution}, tous les boutons sont actifs, et le bouton \textit{Démarrer} devient \textit{Arrêter}. 
\begin{figure}[h!]
\raggedleft
...
En mode édition, le bouton \textit{Démarrer} lance l'assemblage du code présent dans l'éditeur. Si aucune erreur n'est détectée, le simulateur passe alors en mode d'exécution. Dans ce mode, le bouton \textit{Arrêter} stoppe la simulation et revient en mode édition. Notez qu'une modification à l'éditeur a le même effet. Les cinq autres boutons sont, respectivement, de gauche à droite :
\begin{enumerate}
	\item \textbf{Réinitialiser} : effectue l'équivalent d'une interruption \emph{reset} sur le microprocesseur. La valeur de PC est mise à 0. Notez que cela n'affecte \emph{pas} la valeur des autres registres et des drapeaux.
